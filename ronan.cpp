...
struct syWRonan : syVRonan {
  ...
  void SetFrame(const Phoneme &p1s, const Phoneme &p2s, const float x, syVRonan &dest) {
    ...
    for (int i = 0; i < 7; i++) dest.rdef[i].set(flerp(*p1f[i], *p2f[i], x) * pitch, flerp(*p1b[i], *p2b[i], x), db2lin(*p1a[i], *p2a[i], x));
...
extern "C" void ronanCBNoteOn(syWRonan *wsptr) { wsptr->wait4on = 0; }

extern "C" void ronanCBNoteOff(syWRonan *wsptr) { wsptr->wait4off = 0; }
...
extern "C" void ronanCBProcess(syWRonan *wsptr, float *buf, uint32_t len) {
  ...
  // prepare interpolation
  {
    ...
    for (uint32_t i = 0; i < (sizeof(syVRonan) / sizeof(float)); i++) dest[i] = (src2[i] - src1[i]) * mul;
  }
  ...
  for (uint32_t i = 0; i < len; i++) {
    // interpolate all values
    {
      ...
      for (uint32_t i = 0; i < (sizeof(syVRonan) / sizeof(float)); i++) dest[i] += src[i];
    }
    ...
    for (int r = 2; r < 7; r++) out = wsptr->res[r].tick(in) - out;
...
extern "C" { extern void *synthGetSpeechMem(void *a_pthis); }
